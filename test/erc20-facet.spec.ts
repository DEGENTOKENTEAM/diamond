...
describe('DegenX (DGNX ERC20)', function () {
  ...
  let diamondAddress: string, deployerAddress: string, minterAddress: string;
  let ERC20Facet: ERC20Facet;
  let minterContract: MinterBurnerMock;
  ...
  beforeEach(async () => {
    ...
    const { address: _minterAddress } = await deployments.deploy('MinterBurnerMock', { from: deployerAddress });
    minterAddress = _minterAddress;
    minterContract = await ethers.getContract('MinterBurnerMock');
  ...
  it('address can mint a specific max amount of tokens', async () => {
    ...
    await expect(
      minterContract['mint(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('1'))
    ).to.revertedWithCustomError(ERC20Facet, 'NotAllowed');

    await expect(
      ERC20Facet.connect(deployer).updateBridgeSupplyCap(deployerAddress, parseEther('1'))
    ).to.be.revertedWithCustomError(ERC20Facet, 'AddressNoContract');
    await expect(ERC20Facet.connect(deployer).updateBridgeSupplyCap(minterAddress, parseEther('1')))
    ...
    const tx = await minterContract['mint(address,address,uint256)'](
      diamondAddress,
    ...
    await expect(
      minterContract['mint(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('0.1'))
    )
  ...
  it('address can burn a specific max amount of tokens', async () => {
    ...
    await (await ERC20Facet.updateBridgeSupplyCap(minterAddress, parseEther('1'))).wait();
    await (
      await minterContract['mint(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('1'))
    ).wait();

    await expect(
      minterContract['burn(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('2'))
    )
      .to.be.revertedWithCustomError(ERC20Facet, 'BridgeSupplyExceeded')
      .withArgs(parseEther('2'), parseEther('1'));

    // allowance
    await (await ERC20Facet.connect(tokenReceiver).approve(minterAddress, parseEther('1'))).wait();
    await expect(
      minterContract['burn(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('1'))
    ...
    await expect(
      minterContract['burnFrom(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('1'))
    )
      .to.be.revertedWithCustomError(ERC20Facet, 'BridgeSupplyExceeded')
    ...
    await (await ERC20Facet.updateBridgeSupplyCap(minterAddress, parseEther('0'))).wait();
    await (await ERC20Facet.connect(tokenReceiver).approve(minterAddress, parseEther('1'))).wait();
    await expect(minterContract.burnFrom(diamondAddress, tokenReceiver.address, parseEther('1'))).to.changeTokenBalance(
      ERC20Facet,
      tokenReceiver.address,
      parseEther('-1')
    );
    ...
    // simple burnFrom
    await (await ERC20Facet.updateBridgeSupplyCap(minterAddress, parseEther('2'))).wait();
    await (
      await minterContract['mint(address,address,uint256)'](diamondAddress, tokenReceiver.address, parseEther('1'))
    ).wait();
  ...
  describe('Post Initialize', () => {
    ...
    describe('Tokenomics', () => {
      ...
      beforeEach(async () => {
        ...
        await (await ERC20Facet.updateBridgeSupplyCap(minterAddress, parseEther('200'))).wait();
        await (
          await minterContract['mint(address,address,uint256)'](diamondAddress, deployerAddress, parseEther('100'))
        ).wait();
        await (
          await minterContract['mint(address,address,uint256)'](diamondAddress, lp.address, parseEther('100'))
        ).wait();
